...
class Raycaster:
    ...
    def is_out_of_bounds(self, posx, posy):
        ...
        if posx <= 0 or posy <= 0:
            return True
        if posx >= SCREEN_DIMS[0] or posy >= SCREEN_DIMS[1]:
            return True
    ...
    def has_wall_at(self, x, y):
        ...
        grid_x = int(x // CELL_DIMS[0])
        grid_y = int(y // CELL_DIMS[1])
    ...
    def find_wall_color(self, x, y):
        ...
        color = self.level_master.map_data[int(y // CELL_DIMS[1])][int(x // CELL_DIMS[0])].color
        return color
    ...
    def cast_ray(self, start_pos, angle, map_shown):
        ...
        if is_facing_up:
            first_intersection_y = (start_pos[1] // CELL_DIMS[1]) * CELL_DIMS[1] - 0.01
        if is_facing_down:
            first_intersection_y = (start_pos[1] // CELL_DIMS[1]) * CELL_DIMS[1] + CELL_DIMS[1]
        ...
        found_horizontal_wall = False
        while -1 <= next_horizontal_x <= SCREEN_DIMS[0] and -1 <= next_horizontal_y <= SCREEN_DIMS[1]:
        ...
        if is_facing_right:
            first_intersection_x = ((start_pos[0] // CELL_DIMS[0]) * CELL_DIMS[0]) + CELL_DIMS[0]
        elif is_facing_left:
            first_intersection_x = ((start_pos[0] // CELL_DIMS[0]) * CELL_DIMS[0]) - 0.01 # L'erreur venait d'ici :3
        ...
        while -1 <= next_vertical_x <= SCREEN_DIMS[0] and -1 <= next_vertical_y <= SCREEN_DIMS[1]:
    ...
    def raycast(self, map_shown: bool) -> tuple:
        ...
        self.raycast_colors = []
        self.rays_final_pos = []
        rayAngle = (self.player.x_angle - HALF_FOV)
